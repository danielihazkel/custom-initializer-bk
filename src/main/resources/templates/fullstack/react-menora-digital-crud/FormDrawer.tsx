...
export function FormDrawer({ open, title, subtitle, onClose, onSave, saving, dirty, children }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape' && !e.defaultPrevented) requestCloseRef.current() }
  ...
  return (
    <div className="fixed inset-0 z-40 flex">
      ...
      <div
        ...
        className={`ms-auto relative flex h-full w-full max-w-md flex-col rounded-s-[20px] bg-surface shadow-2xl transition-transform duration-200 ease-out ${
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border bg-surface-2 px-5 py-3">
            <Button variant="text" label={t('cancel')} onClick={requestClose} />
            {/* The yellow pill stays a native type="submit" (the Button port only renders type="button"). */}
            <button
              type="submit"
              disabled={saving}
              className="mn mn-btn mn-btn--primary mn-btn--compact"
            >
