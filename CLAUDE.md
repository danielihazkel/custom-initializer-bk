...
- **Primary keys** — at least one PK; **composite keys** are supported (multiple `primaryKey` fields). A composite key renders a `@IdClass(<Entity>.<Entity>Id.class)` plus a nested `static <Entity>Id implements Serializable` key class (Lombok variant uses `@Data`/`@NoArgsConstructor`/`@AllArgsConstructor`), the repository/service id type becomes the key class, and the controller addresses rows by ordered path segments `/{k1}/{k2}` (precomputed as `pkPath`). The frontend `useResource` `update`/`remove` accept an ordered key array and join it (`toPath`). A `generated` PK requires a **single** integral PK (composite + generated is rejected). `EntityScaffoldContext` exposes `pkField` (first PK, back-compat), `pkFields` (list), `hasCompositePk`, `keyClassName`, `pkType`, `pkPath`.
