...
- *Roles:* any page but the start page and a tab target may take `roles` (`ADMIN`/`USER`, the ldap-auth `Constants`), which needs `ldap-auth`/`ldap-auth-rest` among the resolved deps (checked in `FullstackStarterController.buildArtifacts` after the set defaults are added). The frontend gets `src/app/access.ts` (`useRoles` from `GET /api/me/roles`, `canSee`), filters the nav by it and shows a "No access" `PageNotice` on a guarded route; the backend sets gain `web/MeController` (gated `hasLdapAuth`, so every ldap-auth fullstack project answers `/api/me/roles`).
