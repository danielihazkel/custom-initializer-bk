...
| `secured` | `optScaffoldSecured` | **Fullstack.** *Backend:* Controller CRUD methods get **commented-out** `@RequiresPermission` hints (reads → `Constants.USER`, writes → `Constants.ADMIN`) plus commented `security.*` imports and a how-to-enable note — uncomment per-endpoint to enforce (emitted commented so the default project compiles with no unused imports / no enforcement); plus a non-perEntity **`MeController`** (`GET /api/me`, gated whole file) that echoes the caller's username + resolved roles via the ldap-auth `PermissionService`. *Frontend:* an identity layer — `src/shared/auth` (`AuthProvider`/`useAuth`/`devIdentity`), a `src/pages/login` gate, a header user-menu + sign-out, and **role-gated write controls** (`canWrite = isAdmin`; New/Edit/Delete and bulk/kanban writes suppressed for non-admins, reusing the read-only-entity `onEdit`/`onDelete`-omission path). The api client sources the dev `userinfo` header from the login-chosen identity (`getDevIdentity()`) and drops to the gate on a 401 (`auth:unauthorized` event). Aligns with the header/SSO model — no credential endpoint. The flag **requires `ldap-auth` on the build** in **both** render contexts (backend `&& containsKey("ldap-auth")`, frontend `&& hasLdapAuth`) — no identity source otherwise; no-ops cleanly if ldap-auth is deselected. Frontend role-gating is faithful-but-cosmetic until the backend guards are uncommented |
| `csvExport` | `optScaffoldCsvExport` | Controller gets a `GET /api/x/export.csv` endpoint (RFC-4180 cells, honors `q`/filters/sort, ignores pagination) and the FE `EntityPage` gets an **Export** button wired to `useResource.exportCsv` (downloads via the `api.download` blob helper). No extra dependency |
