...
**Frontend page layouts (`pages`).** A fullstack request may carry `pages` (`FullstackStarterRequest.PageDefinitionDto`): the generated frontend's screens as a fixed catalog of page types — `entity-list` (`entity`, optional `presetFilter` of enum/boolean field → value), `dashboard` (`widgets`: `kpi` one number, `bar` grouped by an enum/boolean `groupBy` — defaults to the first enum, else boolean — `line` a time series over a temporal `groupBy` bucketed by `bucket` (`day`/`month`/`year`, default month), and `recent` latest `limit` rows, newest first by `sortBy` — any non-key field, default the PK; `kpi`/`bar`/`line` take `agg` (`count` default, else `sum`/`avg`/`min`/`max`) over a non-key numeric `field`; every widget may take a `span` of 1–4 grid columns — default 1 for a tile, 2 otherwise, rendered as a literal `className` on the widget — and a `presetFilter` like a list page's. A dashboard `dateRange` (`all`/`7d`/`30d`/`90d`/`ytd`/`12m`) adds a `PeriodSelect` opening on that period; each widget's `dateField` — default its entity's first filterable non-key date, which must be filterable because the period travels as the list's `<field>From`/`<field>To` params (`stats.ts` `rangeParams`, whole days for a date-time) — is limited to it, a widget whose entity has no such date is not, and a `dateRange` that limits no widget is a 400. The widgets take one `params` string (preset + period, joined by `statsQuery`) that both the `/stats` calls and the list calls carry. Two more widget kinds: `top` ranks the largest `limit` groups of an enum/boolean field **or a MANY_TO_ONE relation** (`groupBy` = the relation's field name; its ids are named from the target's list by `TopList`), and `progress` draws an aggregate against a positive `target`. A `kpi` with `compare: true` — which needs the dashboard `dateRange` and a widget date — also fetches the previous period (`rangeParams(…, true)`; none under "all time") and shows the change. **Drill-down:** a bar, a ranked row or a trend point opens the entity's first visible list page (`PageLinks.listPageOf`) with the group as a route filter — `{field: key}`, `{<relation>Id: key}` or `stats.ts` `bucketRange` for a date bucket — plus the widget's own params (`queryOf`), but only when the list can filter by that key (`putDrill`); "View all" on a filtered widget opens that list with its filters too), `tabs` (2–6 `tabs`, each embedding another page by id — never a tabs or record page; requires a `title`), `master-detail` (a `parent` list beside the `child` rows of the selected parent, linked by the child's `via` MANY_TO_ONE — defaulted when the child has exactly one relation to the parent, required and named in the error when it has several) `record` (one row of `entity` opened by id, with a details tab plus one `childTabs` entry per related entity — the default is every entity with a MANY_TO_ONE to it — and up to four `headerStats` tiles above the tabs: `{child, agg?, field?, title?}` over the record's rows of a related entity, filtered through the child's relation param; default one row count per related tab, `[]` for none; a tile whose entity is a tab opens it) and `report` (one `entity`'s filter bar, a `chart` — or `charts`, 1–4 of them; giving both is a 400 — each `groupBy` an enum/boolean for bars or a date for a line, with the same `agg`/`field`/`bucket`, drawn by the shared `widgets.tsx` `ReportChart` (the first with the grouped totals table, the rest titled in a grid below; a click drills into the list with the report's filters), and an Export CSV button when the `csvExport` opt is on), and `wizard` (a create form for a writable `entity` split into up to eight `steps` of `{title?, fields}` — field names, plus relation field names for the pickers — then a review (`<Entity>Detail`), posting to the list endpoint and landing on the new row's record page, else the entity's home, else a fresh wizard; every required field and relation must be asked for exactly once, and an omitted `steps` deals the form four fields to a step, relations last; one wizard per entity. `ScreenWizard.tsx.mustache` drives the entity's own `<Entity>Form`, which takes an `only` prop (the step's fields, `{show(...) && …}` around each field) **only for an entity with a wizard** — the per-entity `formHasSteps`/`hasWizardPage`, read in `buildEntityContext` from the page context's `__wizardPages` — so every other form keeps its bytes; that entity's page-mode `EntityPage` also takes `onCreate`, and its list screens send New to the wizard. A hidden wizard stays a route). Every page has a slug `id` (→ `src/app/screens/<PascalId>Screen.tsx`), optional `title`/`description`, and `hidden` (out of the nav, reachable only as a tab). A nav page may also name a `group` (≤40 chars; pages sharing it are listed together as one nav section, gathered where the group first appears — a labelled sidebar section in the tailwind shell, a caret item opening a `DropdownMenu` in the Menora top bar) and an `icon` (one of the lucide names in `FullstackPageValidator.NAV_ICONS`, matched ignoring case; default: the type's own). Either on a hidden page is a 400. A record page is **always** hidden — it needs a record id — so `hidden: false` on one is a 400, and an entity gets at most one; a master-detail parent and a record entity both need a single PK. `FullstackPageValidator` checks references against the converted entities and canonicalizes names/enum constants (400 naming the page). **No pages → the classic shell, byte for byte**: `EntityScaffoldContext.putPageContext` only sets `hasPages=false`, and `App.tsx` / `EntityPage.tsx` keep their old output under `{{^hasPages}}`. With pages, `putPageContext` (frontend context only) adds `pages`/`navPages`/`routePages` (nav + record pages, what the shell can show)/`recordPages`/`initialPageId`/`navIconImports` (the sorted lucide import list)/`navGroups` (+ `hasNavGroups`)/`hasDashboardPages`/`hasReportPages`/`hasWidgets`/`hasRecordPages`/`hasNavigatingScreens` and a per-page view-model (`PageName`, `pageTitleExpr` — a ready TS expression, user text escaped by `tsString` — `pageIs*`, `needsNavigate`, `usesT`, widget/tab/childTab lists, and the `PageLinks` lookups `hasRecordPage`/`recordPageId`/`recordPk` — where rows of an entity open — and `hasBack`/`backPageId`, an entity's visible list, master-detail or tabs "home"). Template files with `perPage=true` (column `per_page`, `V22`) render once per page against project context + page view-model, gated per type with the ordinary `gatedBy` (`pageIsEntityList`/`pageIsDashboard`/`pageIsTabs`/`pageIsMasterDetail`/`pageIsRecord`/`pageIsReport`); `widgets.tsx` (the shared charts and tiles) and `stats.ts` (the `/stats` client — kept separate because the generated lint's react-refresh rule lets a component file export only components) are gated on `hasWidgets`, which a report sets as well as a dashboard. Screens import `t`, enum `…Labels` and take `onNavigate` only when used (the generated lint rejects unused imports). In page mode `EntityPage` takes props (`<Entity>PageProps`): `initialFilters` (preset-filtered list pages) when `hasFilters`, `onOpenRecord` (a row opens its record page instead of the quick-look drawer), and — when `pageScopeable`, i.e. the entity has relations (set in `buildEntityContext`) — `scope` ({param, value}: a pinned relation filter that is merged into the query, hidden from the `FilterBar` and pre-filled into new records, so New inside a master-detail/record page links the row to its parent). **Routing is the URL hash** (`#/<page>[/<arg>][?<field>=<value>&…]`): `src/app/route.ts` (`route.ts.mustache`, gated `hasPages`, borrowed by the Menora set) holds the `View` union, `parseRoute`/`toHash` and a `useRoute()` hook that follows `hashchange`, so Back/Forward, reloads and bookmarks work; an empty hash — or a record page without an id — opens `initialPageId`, and an unknown page sets `route.notFound`, which the shell shows as a `PageNotice` with a way back to the start page. The shell hands screens one `goView(view, arg?, query?)`; each screen also gets its route props from the per-page `routeProps` string: a tabs page `tab`/`onTabChange` (the open tab is the route arg, `#/queue/tickets-open`) and a filterable list page (`listTakesQuery`) `filters` from the query, which it merges over its preset and keys the `EntityPage` on, so a new filter set is a fresh list. `react-menora-digital-crud` authors only `ScreenTabs` (the Menora `Tabs` port) and borrows the other screens + widgets — including `ScreenReport`, which picks up the Menora `FilterBar` for free. Examples carry optional `pages` + `settings` (JSON columns, `V22`; `settings` = `locale`/`dashboardTitle`/`dashboardOverview`/`backendTemplateSet`/`frontendTemplateSet`/`colorPalette`/`scaffold[]`), validated by `FullstackExampleAdminController.validatePages`/`validateSettings` on save and import and included in export. **Upgraded DBs** keep the pre-`V22` template rows (no screens, no `hasPages` branch) until the sets are re-seeded — a layout request there silently renders the classic shell. Pinned by `FullstackPagesIntegrationTests`; built for real by `GeneratedFrontendBuildSmokeTests.fullstack*PageLayouts*` (Orders on the tailwind set for the id-driven screens, Tickets on the Menora set for its Tabs port) and `fullstack*ReportLayout*` (Sales reporting on both sets, for the aggregate widgets and the report screen). Every `fullstackFrontend*` smoke test also runs `eslint --max-warnings 0` over the whole generated frontend — the generated pre-commit hook runs the same lint, so a template that leaves an unused import/local (e.g. the unselected i18n table, now exported as `STRINGS`) fails there.

**Page layout additions.**
- *Widgets:* `donut` (a `bar` drawn as a ring — `BreakdownCard donut`), `stacked` (a breakdown whose bars are split by a second enum/boolean `series`, default the next one; `StackedCard` reads `/stats?groupBy=…&series=…`, which the generated `Service.stats` answers with one bucket per (group, series) pair — `StatsBucket.series`, `@JsonInclude(NON_NULL)` so unsplit responses are unchanged; `series` with `top` or a date `groupBy` is a 400) and `text` (no entity: `title?`, `text` ≤ 2000 chars split into paragraphs on blank lines, `span`; anything else on it is a 400).
- *Record pages:* `childTabs` entries are an entity name or `{entity, via}` (`ChildTabDto`, a bare string still parses), and `headerStats` take `via` — the child's relation to the record entity, default its first (a tile without `via` follows its tab's link). A writable record's page has Edit and Delete: Edit opens the form drawer, or — when the entity has a wizard page — `#/<wizard>/<id>` (`recordEditsInWizard`/`recordEditsInDrawer`/`recordMutable`).
- *Wizard edit:* a single-key entity's wizard (`wizardEditable`) takes `editId` from the route arg: it loads the row, runs the same steps with `isNew=false` and saves with a PUT, landing on the record page (else `history.back()`).
- *Master-detail* keeps the selected parent in the route arg (`selectedId`/`onSelect` route props; local state when embedded as a tab).
- *Period picker* adds "Custom": a `custom:<from>:<to>` `Period` (`customPeriod`/`customRange` in `stats.ts`; its previous period is the equal-length stretch before it).
- *Loading/errors:* widgets, the report chart, record and master-detail screens show `Skeleton`s and a "Try again" (`useStats`/`useNumber` return `retry`).
- *Shell:* off-nav routes (record pages, hidden wizards — `offNavPages`) are titled in the header, and one with a home page shows it as a breadcrumb (`crumbPages` → `RECORD_PARENTS`).
- *Roles:* any page but the start page and a tab target may take `roles` (`ADMIN`/`USER`, the ldap-auth `Constants`), which needs `ldap-auth-rest` or `ldap-auth` among the resolved deps (checked in `FullstackStarterController.buildArtifacts` after the set defaults are added — both backend sets default to `ldap-auth-rest`, so a default project can use roles). The frontend gets `src/app/access.ts` (`useRoles` from `GET /api/me/roles`, `canSee`), filters the nav by it and shows a "No access" `PageNotice` on a guarded route; the backend sets gain `web/MeController` (gated `hasLdapAuth`, so every ldap-auth fullstack project answers `/api/me/roles`).
- *List presentation:* an `entity-list` page may say how it opens — `columns` (an ordered subset of the entity's columns: field names, MANY_TO_ONE field names and, only when the `audit` opt applies to the entity, `createdAt`/`updatedAt`; the validator takes the request's `opts.scaffold` — an example's `settings.scaffold` — for that), `sort` (`{field, dir}`; the generated controller's `SORTABLE` = every field + the audit pair, never a relation), `view` (one of the entity's *emitted* views — `EntityScaffoldContext.emittedListViews`, shared by the validator and the renderer, so a kanban on a read-only entity or a calendar without a date is a 400 naming the page) and `pageSize` (`10`/`20`/`50`/`100`, the pager's own choices). The screen passes them as literal props (`columns`, `initialSort`, `initialView`, `initialPageSize`); `EntityPage` takes them **only for an entity whose list page sets one of them** (`listConfigurable`, from `__listPresentationEntities` in the page context — the `hasWizardPage` pattern), where its columns gain a `key` (`ListColumn`) and `shown` is the picked subset. Every other entity's page, and the classic shell, keep their bytes. Pinned by `fullstackEndpoint_listPagePresentation*`/`_rejectsInvalidListPresentation`; the Inventory example's items page uses it, built by `GeneratedFrontendBuildSmokeTests.fullstack*ListPresentation*`.
- *Connected pages:* a relation whose target entity has a record page renders as a link to it (`#/<record>/<id>`) in the table cell and the detail row — `buildEntityContext` copies each relation view-model with `targetHasRecordPage`/`targetRecordPageId` from the `__recordPages` lookup, which only the frontend page context carries, so backend contexts and layout-free frontends keep their bytes. In page mode every `EntityPage` takes `title`/`description`/`embedded`: a list page's own title and description replace the entity heading (`hasPageTitle`), and a list embedded as a tab (`isTabTarget` → the screen's `embedded` prop), a record's related list or a master-detail child drops its heading altogether. A tabs page shows its title and description above the strip, and each tab over a list page shows that list's row count (`tabCounted`/`countPath`/`countParams` — the preset spliced with a triple-stash — fetched with `?size=1` by a per-screen `useTabCounts` hook, refetched on tab change; the Menora `Tabs` port takes string labels, so the count rides inside the label). A dashboard with a period picker and a filterable report keep that state in the route query (`#/desk?period=30d`, `#/revenue?region=NORTH`): `useRoute()` returns a third `replace` (`history.replaceState`, no history entry), the shell derives `setQuery` when a routed screen `takesQuery` (`hasQueryScreens`), and the screens take `period`/`onPeriodChange` or `query`/`onQueryChange` as optional props — embedded as a tab they fall back to local state, the master-detail pattern (`parsePeriod` in `stats.ts`; a report keeps a cleared preset field in the route so it stays cleared). With soft delete, a record page's Delete toast offers Undo (`recordSoftDeletes` — the entity's `softDelete` override resolved like `csvExportOverride` — posts `/restore` and reopens the row). Pinned by `…rendersPageTypes`, `…rendersMasterDetailAndRecordPages`, `…recordPageOffersUndoAfterASoftDelete`, `…rendersReportPageAndAggregateWidgets`; the Orders smoke build runs with `softDelete` on for the Undo path.
- *Editor:* the UI mirrors every rule above in `pageLayout.ts` (`validatePages(pages, entities, { ldapAuth })`), keeps settings across a widget kind/entity switch (`retargetWidget`/`retargetReport`/`retargetWizardSteps`, naming what was dropped with an undo), and maps a Generate **or Explore** 400 naming a page onto that page (`pageOfServerError`). The Pages section sits below Entities (both in the section nav). "Add widget" opens a gallery (entity + kind, each kind disabled with the validator's reason — `widgetKindDisabledReason`, also on the card's kind switcher; `newWidget` pre-fills a progress target / note text). A list page's "Split into tabs" (`splitListByField`, `splitDisabledReason`) makes one hidden preset list per enum/boolean value under a new tabs page as one undo step. A hand-edited page id is kept on the page as `idLocked` (editor-only — `requestPages` strips it from every request and from the admin example's stored JSON), so it survives a reload or a load. The command palette offers Add page / Start from my entities / Use classic layout / Open the layout preview (`addRequest`/`previewRequest` props).

Pinned by `FullstackPagesIntegrationTests` (`…DonutStackedAndTextWidgets`, `…recordTabsAndTilesLinkThroughTheNamedRelation`, `…restrictsPagesToRolesWithLdapAuth`, `…shellShowsNotFoundAndRecordBreadcrumbs`). The Blog and Inventory examples ship layouts that use them; Course enrolments stays layout-free to show the classic shell.

**Generated aggregation endpoint (`GET /api/x/stats`).** Every entity with something to group,
bucket or reduce gets one: `?groupBy=` (an enum/boolean column, a MANY_TO_ONE relation — grouped by the id it points at,
`root.get(rel).get(pk)`, so no join — or a date column with
`&bucket=day|month|year`), `&agg=count|sum|avg|min|max` over `&field=`, `&top=1..100` (the largest
groups only, biggest first, via `setMaxResults`; a date or missing `groupBy` is a 400), plus the entity's own `q`
and filter params — so a chart always agrees with the list beneath it. It answers
`{"buckets":[{"key","value"}],"total"}` (`total` skips empty groups — an avg over no rows is `null`, never the NPE it used to be — and is `null` when every group is empty); `key` is the enum constant, `true`/`false`, a date bucket
