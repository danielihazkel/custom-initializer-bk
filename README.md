...
10. [Fullstack CRUD Generator](#fullstack-crud-generator)
    - [End-to-End Flow](#end-to-end-flow)
    - [Request Wire Format](#request-wire-format)
    - [Dependency Picker & Defaults](#dependency-picker--defaults)
    - [Import From DDL](#import-from-ddl)
...
## Fullstack CRUD Generator

A separate top-level **Fullstack** tab next to Backend / Frontend that takes a list of user-defined entities (name + primitive fields) and emits a single ZIP containing a working Spring Boot backend (`backend/`) plus a working React frontend (`frontend/`) — already wired together end-to-end. Each entity becomes a `@Entity` / `Repository` / `Dto` / `Service` / `@RestController` set on the backend (with server-side pagination, sort, and search) and a Table + Form CRUD page on the frontend.
...
POST /starter-fullstack.zip
   │
   ├─ FullstackRequestValidator (Java identifiers, one PK per entity, ENUM values, length-on-String, …)
   ├─ ensureRequiredDeps         (merges set defaults if caller didn't supply dependencies)
   ├─ Spring Initializr pipeline (renders backend via EntityScaffoldContributor)
   ├─ FullstackRenderer          (renders frontend inline, outside the pipeline)
   └─ zip → backend/ + frontend/ + README.md + .gitignore
```

The backend goes through the standard Initializr per-request child context — the new `EntityScaffoldContributor` (registered in `META-INF/spring.factories`) picks up the entity list from a `EntityDefinitionContext` ThreadLocal and renders the per-entity template files into the generated project. The frontend is rendered inline because the Initializr framework is Java-project-shaped and doesn't know about Vite.

A `POST /starter-fullstack.preview` endpoint returns the same file tree as JSON for the Explore modal.
...
```json
{
  "groupId": "com.menora",
  "artifactId": "shop",
  "packageName": "com.menora.shop",
  "bootVersion": "3.2.1",
  "javaVersion": "21",
  "dependencies": ["data-jpa", "web", "h2", "validation", "actuator"],
  "backendTemplateSet": "spring-jpa-crud",
  "frontendTemplateSet": "react-tailwind-crud",
  "entities": [
    {
      "name": "Product",
      "tableName": "products",
      "fields": [
        ...
        { "name": "sku",   "type": "String",     "required": true, "unique": true, "length": 64 },
        { "name": "name",  "type": "String",     "required": true, "length": 255 },
        { "name": "price", "type": "BigDecimal", "required": true },
        { "name": "status","type": "Enum",       "enumValues": ["ACTIVE","DISCONTINUED"] }
      ]
    }
  ]
}
...
Supported `type` values (v1, primitives only — no relationships): `String`, `Long`, `Integer`, `Boolean`, `LocalDate`, `LocalDateTime`, `BigDecimal`, `Enum`. `length` is only valid on `String`; `enumValues` is required and only valid on `Enum`.

**Example — generate a shop:**

```bash
curl -X POST http://localhost:8080/starter-fullstack.zip \
  -H "Content-Type: application/json" \
  -d '{
    "artifactId": "shop",
    "packageName": "com.menora.shop",
...
unzip -l shop.zip   # → shop/backend/... shop/frontend/... shop/README.md
```

### Dependency Picker & Defaults

The Fullstack wizard exposes a **Dependencies** section seeded from the selected backend template set's admin-configured defaults. Nothing is forced — the user can uncheck *anything* before generating. Wire-level semantics:

| `dependencies` field in request | Behavior |
|---------------------------------|----------|
| **omitted** (null) | Controller falls back to the set's default deps — convenient for API users who don't read `/metadata/entity-template-sets` first |
| **`[]` (empty list)** | Respected exactly — generated project has no extras |
| **`["a","b","c"]`** | Respected exactly — no merging with defaults, no auto-additions |

The UI always sends the user's final selection here, so unchecking a default in the picker actually loses it. The previous "always-add data-jpa/web/h2" safety net has been removed — admins decide what each set ships pre-checked.

`spring-jpa-crud` is seeded with `["data-jpa", "web", "h2", "validation", "actuator"]`. Drop `h2` and add `postgresql` (via the admin tab) to ship a postgres-only set.

### Import From DDL

For users with an existing schema, a drawer on the Entities step accepts pasted `CREATE TABLE` statements and pre-fills the entity editor. Powered by the existing SQL wizard's parser (`SqlEntityGenerator.parseTablesForImport`), so dialect-specific types (`BIGSERIAL`, `NUMERIC(p,s)`, `NVARCHAR`, …) just work.

```bash
curl -X POST http://localhost:8080/metadata/fullstack/import-ddl \
  -H "Content-Type: application/json" \
  -d '{
    "dialect": "POSTGRESQL",
    "sql": "CREATE TABLE products (id BIGSERIAL PRIMARY KEY, sku VARCHAR(64) NOT NULL, price NUMERIC(10,2));"
  }'
# → {"entities":[{"name":"Product","tableName":"products","fields":[...]}]}
```

Mapping is intentionally lossy and a starting point — the user reviews the result in the editor before generating. Notable degradations: `UUID` → `String`, `Short` → `Integer`, `Float`/`Double` → `BigDecimal`, `byte[]` → `String`. Column-level `UNIQUE` and `CHECK (col IN (...))` constraints are not parsed; toggle `unique` and switch types to `Enum` manually after import.

Parse errors return HTTP 400 with `{error, detail, statementIndex, snippet}` — the drawer renders these inline.
...
|------|-------------|---------|
| `BACKEND_JAVA` | `EntityScaffoldContributor` inside the Initializr pipeline | Per-entity `.java` files; gets dep resolution + pom assembly for free |
| `FRONTEND_REACT` | `FullstackRenderer` inline | `package.json`, `vite.config.ts`, per-entity `.tsx` pages, shared `Table.tsx` / `FormDrawer.tsx` |

Each file in a set carries a `pathTemplate` (Mustache-rendered, so `src/main/java/{{packagePath}}/{{EntityName}}Controller.java` becomes one file per entity) and a `perEntity` flag — `true` renders once per user entity; `false` renders once total (e.g. `App.tsx`, `Table.tsx`).

The Mustache context exposes:
...
| `EntityName`, `entityName`, `entity_name`, `entityNameKebab`, `EntityNamePlural`, `entityNamePluralKebab` | Per-entity naming variants |
| `tableName` | `entity.tableName` or pluralized snake_case of `EntityName` |
| `fields` | `List` with `name`, `Name`, `column`, `javaType`, `tsType`, `isString`/`isNumeric`/`isBoolean`/`isTemporal`/`isEnum`, `isPrimaryKey`, `isGenerated`, `isRequired`, `isUnique`, `hasLength`, `length`, `enumValues` |
| `stringFields`, `hasStringFields` | Filtered view used for the cross-field search Specification |
| `pkField`, `nonPkFields` | Convenience splits |
| `imports` | De-duplicated FQN list for the entity file's import block |

The seeded `spring-jpa-crud` set's Service/Controller templates use `hasStringFields` to render a JPA `Specification` LIKE chain across STRING columns for the `?q=` search parameter; entities without any STRING field fall through to plain `repository.findAll(pageable)`.

### Fullstack Admin Endpoints

Full CRUD on the template sets and their per-set defaults. All require the admin token.

```bash
GET    /admin/entity-template-sets                  # list
POST   /admin/entity-template-sets                  # create (returns the new set incl. id)
PUT    /admin/entity-template-sets/{id}             # update
DELETE /admin/entity-template-sets/{id}             # cascade-deletes files + default-deps

GET    /admin/entity-template-files?setId={id}      # files in set
POST   /admin/entity-template-files                 # create
PUT    /admin/entity-template-files/{id}            # update
DELETE /admin/entity-template-files/{id}            # delete

GET    /admin/entity-template-sets/{id}/default-deps   # → ["data-jpa","web",…]
PUT    /admin/entity-template-sets/{id}/default-deps   # body: {"depIds": ["…"]} — full replace
```

The public `/metadata/entity-template-sets` endpoint surfaces enabled sets to the wizard, each with its `defaultDeps` so the UI can pre-check the right boxes when a user picks the set.

Configuration export/import covers the new tables — `/admin/export` round-trips `entityTemplateSets`, `entityTemplateFiles`, and `entityTemplateSetDefaultDeps`.
