...
10. [AI File Assistant (free-form file generation)](#ai-file-assistant-free-form-file-generation)
...
## AI File Assistant (free-form file generation)

Off by default. When enabled, after the developer picks dependencies they can open the **AI File Assistant** panel in the project form, type a free-form prompt (e.g. *"Add a HelloController and a small DateUtils helper"*), and the backend forwards the prompt to a configured AI endpoint, parses the proposed files out of the response, and returns them to the UI for review. The developer can remove individual files from the proposed set; whatever survives is bundled into the generated ZIP alongside the standard files.

### Why It Exists

The dependency catalog and the SQL/OpenAPI/WSDL wizards cover everything that can be derived deterministically from structured inputs. The AI assistant is the escape hatch for one-off boilerplate that no catalog can predict: example controllers, demo utility classes, scaffolding the developer would otherwise paste in by hand.

### No AI SDK — Plain REST

The implementation calls the configured endpoint with a plain `RestClient` (the Spring 6.1 HTTP client). There is no Anthropic, OpenAI, or other AI SDK on the classpath. Switching providers is a YAML change — point `ai.endpoint-url` at any service that speaks the OpenAI-compatible `chat/completions` shape (most public APIs and most internal LLM proxies do; Ollama and vLLM speak it natively).
...
```json
{
  "model": "gpt-4o-mini",
  "messages": [
    { "role": "system", "content": "<ai.system-prompt>" },
    { "role": "user",   "content": "Project metadata:\n- groupId: com.menora\n- artifactId: demo\n- packageName: com.menora.demo\n- bootVersion: 3.2.1\n- javaVersion: 21\n- packaging: jar\nSelected dependencies: web, kafka\n\nUser request:\nAdd a HelloController returning a friendly greeting." }
  ]
}
```

The service expects the response to follow the OpenAI `chat/completions` shape and reads `choices[0].message.content`. The content itself must be a JSON object: `{"files":[{"path":"src/main/java/...","content":"..."}]}`. A leading/trailing ```json …``` markdown fence is stripped defensively before parsing.

### Path Safety

Every file the AI returns is validated before it is offered to the UI. The same validator runs again inside the `aiFileContributor` bean as defense in depth. Rejection rules:

- Paths must be relative (no leading `/`, no `C:\` drive letters).
- Paths must not contain `..`.
- Paths must not exceed 500 characters.
- Paths must not target a framework-managed file: `pom.xml`, `mvnw`, `mvnw.cmd`, `build.gradle*`, `settings.gradle*`, `gradlew*`, `src/main/resources/application.{yml,yaml,properties}`, or anything under `.mvn/` or `.gradle/`.

Rejected entries are logged and dropped — they don't fail the generation.
...
`POST /ai/generate-files` body:

```json
{
  "form": {
    "groupId":"com.menora", "artifactId":"demo", "name":"demo", "description":"...",
    "packageName":"com.menora.demo", "bootVersion":"3.2.1", "javaVersion":"21", "packaging":"jar"
  },
  "dependencies": ["web", "kafka"],
  "selectedOptions": { "kafka": ["consumer-example"] },
  "prompt": "Add a HelloController and a small DateUtils helper."
}
```
...
{
  "files": [
    { "path": "src/main/java/com/menora/demo/HelloController.java", "content": "package com.menora.demo;\n..." },
    { "path": "src/main/java/com/menora/demo/util/DateUtils.java",  "content": "package com.menora.demo.util;\n..." }
  ]
}
```

Error responses use the same `{error, detail}` JSON envelope as the rest of the wizard endpoints:
...
### Bundling Into the ZIP

Once the user has reviewed and (optionally) removed files in the panel, the standard Generate button posts the kept files to `/starter-wizard.zip` in the new `aiFiles` field. The `aiFileContributor` bean (ordered after the SQL/OpenAPI/SOAP contributors) writes them into the project tree before zipping. The same generation pipeline applies, so AI files happily co-exist with SQL/OpenAPI/SOAP wizard output and standard catalog files in a single Generate click.

### Generate from curl

Skip the UI entirely: call `/ai/generate-files`, then post the response back through `/starter-wizard.zip`:

```bash
# 1. Ask the AI for files
...
# 2. Bundle them straight into the ZIP (jq merges the AI files into the wizard body)
...
unzip -l demo.zip | grep HelloController
```

### Limitations (v1)

- **Single-project only.** The multi-module ZIP path (`GET /starter-multimodule.zip`) does not yet accept a JSON body, so AI files there require a follow-up endpoint.
- **No content editing in the UI.** The user can remove files from the proposed set but cannot edit content in-place — accept-or-reject only. Use a regular IDE for edits after download.
- **Fixed request shape.** The OpenAI-compatible `{model, messages}` body is hard-coded; an org with a non-conforming endpoint should put a thin proxy in front.
- **Generated files are not persisted.** Each session re-asks the AI; nothing is cached server-side.
