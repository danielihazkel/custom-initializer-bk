...
| `enumValues` | List | `{ value, label, labelTs, last }` per enum constant — `label` is the user's `enumLabels` entry or the humanized constant (`IN_PROGRESS` → `In progress`), `labelTs` the same escaped for a single-quoted TS literal |
| `hasEnumLabels` | boolean | true when at least one constant carries a user-supplied label |
